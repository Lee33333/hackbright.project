...
def load_doctors(session):
    ...
    with open('seed_data_simple.csv', 'rb') as user_file:
        ...
        for row in reader:
            ...
            doctor = model.Doctor(name=row[0],cert=row[1],business_name=row[2],address=row[3],suite=row[4],phone_number=row[5],recommended_by=row[6],gender=row[7],lat=row[8],lon=row[9],medical=row[10],therapy=row[11],repro=row[12],trans=row[13],pub_insurance=row[14], specialties=row[15],)
            session.add(doctor)
        session.commit()
